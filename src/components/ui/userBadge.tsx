...
const UserBadge = () => {
  ...
  const { data, isLoading } = useAuthVerifyHooks();
  return (
    <div className="flex items-center space-x-4">
      <Button className="relative overflow-hidden w-12 h-12 p-0 border-0 box-content shadow-none rounded-full">
        <span className="sr-only">Toggle user menu</span>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            {!isLoading && (
              <Button
                className="rounded-full w-10 h-10 p-0 absolute top-1/2 transform -translate-y-1/2 left-1/2 -translate-x-1/2"
                size="icon"
                variant="ghost"
              >
                <img
                  alt="User avatar"
                  className="rounded-full object-cover border border-gray-200 w-10 h-10 ring-2 ring-transparent"
                  src={
                    data?.role === "user"
          ...
          <DropdownMenuContent alignOffset={-10} className="mt-1 w-48">
